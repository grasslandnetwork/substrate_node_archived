...
```sh
./target/release/node-grassland -h
```
...
```bash
./target/release/node-grassland purge-chain --dev
```

Start the development chain with detailed logging:

```bash
RUST_BACKTRACE=1 ./target/release/node-grassland -ldebug --dev
```
...
$ ./target/release/node-grassland --dev --base-path ./my-chain-state/
...
```shell
./target/release/node-grassland --help
```
...
(`cargo build --release && ./target/release/node-grassland --dev --ws-external`)
by appending your own. A few useful ones are as follow.

```bash
# Run Substrate node without re-compiling
./scripts/docker_run.sh ./target/release/node-grassland --dev --ws-external

# Purge the local dev chain
./scripts/docker_run.sh ./target/release/node-grassland purge-chain --dev
